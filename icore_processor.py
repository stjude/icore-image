...
def get_dcmtk_dict_path():
    if getattr(sys, "frozen", False):
        bundle_dir = os.path.abspath(os.path.dirname(sys.executable))
        return os.path.join(
            bundle_dir, "_internal", "dcmtk", "share", "dcmtk-3.6.9", "dicom.dic"
        )
    else:
        dcmtk_home = os.environ.get("DCMTK_HOME")
        if dcmtk_home is None:
...
def create_analyzer_engine():
    if getattr(sys, "frozen", False):
        ...
        model_path = os.path.join(
            bundle_dir, "_internal", "en_core_web_sm", "en_core_web_sm-3.7.1"
        )
    ...
    mrn_recognizer = PatternRecognizer(
        ...
        patterns=[
            ...
            Pattern(
                name="mrn_prefix_pattern", regex=r"\b[A-Z]{2,6}-\d{4,10}\b", score=0.7
            ),
        ],
    )
    ...
    title_name_recognizer = PatternRecognizer(
        ...
        patterns=[
            Pattern(
                name="dr_name_pattern", regex=r"(?<=Dr\.\s)([A-Z][A-Z]+)\b", score=0.85
            ),
            Pattern(
    ...
    ssn_recognizer = PatternRecognizer(
        ...
        name="ssn_recognizer",
        patterns=[
            Pattern(name="ssn_pattern", regex=r"\b\d{3}-\d{2}-\d{4}\b", score=0.95),
        ],
    )
...
def ctp_post(url, data):
    ...
    response = requests.post(
        ...
        auth=("admin", "password"),
        data=data,
        headers={"Referer": f"http://localhost:50000/{url}"},
    )
...
def count_files(path, exclude_files):
    return sum(
        len([f for f in files if not f.startswith(".") and f not in exclude_files])
...
def run_progress(data):
    received = (
        ctp_get_status("Files received")
        if data["querying_pacs"]
        else data["dicom_count"]
    )
    quarantined = count_files(
        os.path.join(APPDATA_DIR, "quarantine"),
        {".", "..", "QuarantineIndex.db", "QuarantineIndex.lg"},
    )
...
def start_ctp_run(tick_func, tick_data, logf, ctp_dir):
    ...
    tick_data = {
        "complete": False,
        "querying_pacs": True,
        "dicom_count": count_dicom_files(INPUT_DIR),
    } | tick_data
    tick_thread = Thread(
        target=tick,
        args=(
...
def finish_ctp_run(ctp_process, tick_thread, tick_data, temp_ctp_dir):
    ...
    try:
        ...
        try:
            ...
            logging.warning(
                "HTTP shutdown did not complete in time, falling back to signals"
            )
    ...
    try:
        ...
        try:
            ...
            logging.warning(
                "CTP process did not terminate after SIGTERM, sending SIGKILL"
            )
            ctp_process.kill()
            ctp_process.wait()
...
def cmove_queries(**config):
    ...
    if config.get("acc_col") is not None:
        ...
        if mrn_col and mrn_col in df.columns:
            acc_mrn = list(
                df[[config.get("acc_col"), mrn_col]].itertuples(index=False, name=None)
            )
        ...
        mrn_dates = list(
            df[[config.get("mrn_col"), config.get("date_col")]].itertuples(
                index=False, name=None
            )
        )
        for i, (mrn, dt) in enumerate(mrn_dates, 1):
            dts = datetime.strftime(
                (dt - timedelta(days=config.get("date_window", 0))), "%Y%m%d"
            )
            ...
            queries.append(query)
            logging.info(
                f"Row {i}: MRN={mrn}, TargetDate={dt.strftime('%Y-%m-%d')}, Window={config.get('date_window')} days"
            )
...
def cmove_images(logf, **config):
    ...
    for pacs in config.get("pacs", []):
        ...
        for i, query in enumerate(queries):
            ...
            for entry in output.split("Find Response:")[1:]:
                ...
                if study_uid and len(study_uid) > 0:
                    ...
                    logging.info(
                        f"  Found StudyInstanceUID: {study_uid}, StudyDate: {study_date}"
                    )
            ...
            if studies_found_this_query == 0:
                ...
                logging.info(
                    f"  Total studies found for this query: {studies_found_this_query}"
                )
        ...
        logging.info(f"\n{'=' * 80}")
        logging.info(
            f"FINDSCU SUMMARY: Found {len(study_uids)} unique studies total from PACS {ip}:{port}"
        )
        logging.info(f"{'=' * 80}\n")
        ...
        while current_moves and retry_count < 3:
            if retry_count > 0:
                logging.info(
                    f"Retry attempt {retry_count} for {len(current_moves)} failed moves"
                )
                time.sleep(5)
            ...
            for i, study_uid in enumerate(current_moves):
                logging.info(f"\n{'=' * 80}")
                logging.info(
                    f"MOVESCU {i + 1}/{len(current_moves)} (Retry {retry_count})"
                )
                logging.info(f"{'=' * 80}")
                cmd = [
                    get_dcmtk_binary("movescu"),
                    "-v",
                    "-aet",
                    aet,
                    "-aem",
                ...
                logging.info(
                    f"Progress: {i + 1}/{len(current_moves)} studies processed in this batch"
                )
...
def save_quarantined_files_log():
    ...
    quarantine_dirs = {
        "ArchiveImportService": os.path.join(
            APPDATA_DIR, "quarantine", "ArchiveImportService"
        ),
        "DicomFilter": os.path.join(APPDATA_DIR, "quarantine", "DicomFilter"),
        "DicomDecompressor": os.path.join(
            APPDATA_DIR, "quarantine", "DicomDecompressor"
        ),
        "DicomAnonymizer": os.path.join(APPDATA_DIR, "quarantine", "DicomAnonymizer"),
        "DirectoryStorageService": os.path.join(
            APPDATA_DIR, "quarantine", "DirectoryStorageService"
        ),
    }
...
def save_linker_csv():
    linker_csv = ctp_post(
        "idmap", {"p": 0, "s": 4, "keytype": "trialAN", "keys": "", "format": "csv"}
    )
    ...
    try:
        ...
        for item in blacklist:
            blacklist_recognizer = PatternRecognizer(
                ...
                patterns=[
                    Pattern(
                        name=f"blacklist_pattern_{hash(item)}",
        ...
        duration_pattern = re.compile(
            r"\b\d{1,3}\s+(weeks?|months?|days?|hours?|minutes?|seconds?|mins?|secs?)\b",
            re.IGNORECASE,
        )
        time_pattern = re.compile(r"\b\d{1,2}:\d{2}(\s*[AP]M)?\b", re.IGNORECASE)
        gestational_age_pattern = re.compile(
            r"\b\d{1,2}\s*weeks?\s*\d*\s*days?\b", re.IGNORECASE
        )
        time_reference_pattern = re.compile(
            r"\b(midnight|noon|morning|evening|afternoon)\b", re.IGNORECASE
        )
        complex_age_pattern = re.compile(
            r"\b\d{1,3}\s+years?,\s*\d{1,2}\s+(months?|days?)\s+old\b", re.IGNORECASE
        )
        ...
        operators = {
            ...
            "ALPHANUMERIC_ID": OperatorConfig(
                "replace", {"new_value": "[ALPHANUMERICID]"}
            ),
            ...
            "US_DRIVER_LICENSE": OperatorConfig(
                "replace", {"new_value": "[DRIVERSLICENSE]"}
            ),
...
def date_shift_text(original_list, deided_list, date_shift_by):
    ...
    for i, (original, deided) in enumerate(zip(original_list, deided_list)):
        ...
        result = deided
        for date in dates:
            shifted = datetime.strptime(date, "%m/%d/%Y") + timedelta(
                days=date_shift_by
            )
...
def imageqr_main(**config):
    def setup_config(temp_ctp_dir):
        ...
        if generated_filter:
            ...
            logging.info(
                "No series/date filter generated (no input.xlsx with date columns)"
            )
        ...
        logging.info("=" * 80)
        with open(
            os.path.join(temp_ctp_dir, "scripts", "dicom-filter.script"), "r"
        ) as f:
            logging.info(f.read())
        logging.info("=" * 80 + "\n")
        ...
        formatted_config = IMAGEQR_CONFIG.format(
            appdata_dir=appdata_abs,
    ...
    with ctp_workspace(imageqr_func, {}, setup_config) as logf:
        ...
        logging.info(
            f"Accessions that failed to process: {', '.join(map(str, failed_accessions))}"
        )
...
def header_extract_main(**config):
    ...
    for batch_start in range(0, total_files, batch_size):
        ...
        for filepath in batch_files:
            ...
            try:
                ...
                headers = {
                    elem.keyword or str(elem.tag): elem.value
            ...
            try:
                ...
                if not first_batch and new_columns:
                    ...
                    existing_df = existing_df.reindex(
                        columns=known_columns, fill_value=None
                    )

                    # Write updated CSV
                    existing_df.to_csv(csv_path, index=False, mode="w")
                    logging.info(
                        f"Updated existing CSV with {len(new_columns)} new columns"
                    )
                ...
                logging.info(
                    f"Processed {len(header_data)} headers in this batch (total: {total_headers_processed})"
                )
    ...
    try:
        ...
        logging.info(
            f"Successfully extracted and saved {total_headers_processed} headers to {excel_path}"
        )
    except Exception as e:
...
def imagedeid_main(**config):
    ...
    def setup_config(temp_ctp_dir):
        ...
        logging.info("=" * 80)
        config_template = (
            IMAGEDEID_PACS_CONFIG if querying_pacs else IMAGEDEID_LOCAL_CONFIG
        )
        appdata_abs = os.path.abspath(APPDATA_DIR)
        output_abs = os.path.abspath(OUTPUT_DIR)
    ...
    with ctp_workspace(
        imagedeid_func, {"querying_pacs": querying_pacs}, setup_config
    ) as logf:
        if querying_pacs:
            failed_accessions = cmove_images(logf, **config)
            logging.info(
                f"Accessions that failed to process: {', '.join(failed_accessions)}"
            )
        else:
            failed_accessions = []
    ...
    total_quarantined = count_files(
        os.path.join(APPDATA_DIR, "quarantine"),
        {".", "..", "QuarantineIndex.db", "QuarantineIndex.lg"},
    )
    logging.info(
        f"PROCESSING COMPLETE - Failed accessions: {len(failed_accessions)}, Quarantined files: {total_quarantined}"
    )
...
def get_rclone_path(config):
    ...
    if storage_location not in rclone_sections:
        raise ValueError(
            f"Storage location '{storage_location}' not found in rclone config"
        )
...
def image_export_main(**config):
    try:
        ...
        cmd = [
            ...
            "/rclone.conf",
            INPUT_DIR,
            f"azure:{container_name}/{site_id}/{project_name}",
        ]
...
def textdeid_main(**config):
    try:
        ...
        deid_data = scrub(
            original_data, config.get("to_keep_list"), config.get("to_remove_list")
        )
        # shifted_data = date_shift_text(original_data, deid_data, config.get("date_shift_by"))
        pd.DataFrame(deid_data).to_excel(
            os.path.join(OUTPUT_DIR, "output.xlsx"), index=False, header=False
        )
        print_and_log("PROGRESS: COMPLETE")
    except Exception as e:
        error_and_exit(f"Error: {e}")
...
def validate_config(config):
    ...
    if (
        config.get("module") in ["imageqr", "imagedeid"]
    ...
    if os.path.exists(os.path.join(INPUT_DIR, "input.xlsx")):
        if config.get("module") in ["imageqr", "imagedeid"]:
            if any(
                [config.get("pacs_ip"), config.get("pacs_port"), config.get("pacs_aet")]
            ):
                error_and_exit(
                    "pacs_ip, pacs_port, and pacs_aet have been deprecated. Please use the pacs list instead."
                )
            ...
            for pacs in config.get("pacs", []):
                if not all([pacs.get("ip"), pacs.get("port"), pacs.get("ae")]):
                    error_and_exit("Pacs details missing in config file.")
            if (
                config.get("application_aet") is None
                or config.get("application_aet") == ""
            ):
                error_and_exit("Application AET missing in config file.")
            if config.get("acc_col") is None and (
                config.get("mrn_col") is None or config.get("date_col") is None
            ):
                error_and_exit(
            ...
            if config.get("date_window") is not None and not isinstance(
                config.get("date_window"), int
            ):
...
def generalmodule(**config):
    ...
    if not MODULES_DIR:
        error_and_exit(
            "ICORE_MODULES_DIR environment variable must be set for custom modules"
        )
    ...
    try:
        ...
        error_and_exit(
            f"Module execution failed with exit code {e.returncode}: {e.stderr}"
        )
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        filename=os.path.join(APPDATA_DIR, "log.txt"),
        level=logging.INFO,
