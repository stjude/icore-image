...
def test_imageqr_accession_wildcard_filtering(tmp_path, orthanc):
    ...
    assert found_accessions == {"ABC001"}, (
        f"Should only find studies with exact AccessionNumber 'ABC001' (after trimming), found: {found_accessions}"
    )
...
def test_imageqr_continues_despite_move_failures(tmp_path, capsys, orthanc):
    ...
    for i in range(3):
        ds = _create_test_dicom(
            f"ACC{i:03d}", f"MRN{i:04d}", f"Smith^John{i}", "CT", "2.0"
        )
        ds.InstanceNumber = i + 1
        _upload_dicom_to_orthanc(ds, orthanc)
    ...
    assert 0 in result["failed_query_indices"], (
        "First query should have failed (zero files)"
    )
    assert 1 in result["failed_query_indices"], (
        "Second query should have failed (exception)"
    )

    captured = capsys.readouterr()
    assert "0 files" in captured.out or "Exception while retrieving" in captured.out, (
        "Should log failures"
    )
...
def test_generate_queries_and_filter_with_fallback(tmp_path):
    ...
    query_params, expected_values, generated_filter = generate_queries_and_filter(
        spreadsheet
    )
    ...
    query_params, expected_values, generated_filter = generate_queries_and_filter(
        spreadsheet, use_fallback_query=True
    )
    assert 'AccessionNumber.contains("ACC001")' in generated_filter
...
def test_find_studies_with_fallback_mock(tmp_path):
    ...
    def mock_find_studies(
        host, port, calling_aet, called_aet, query_params, return_tags
    ):
        if "AccessionNumber" in query_params:
            if "ACC001" in query_params["AccessionNumber"]:
                return [
                    {
                        "StudyInstanceUID": "STUDY_UID_1",
...
def test_save_failed_queries_csv_with_fallback(tmp_path):
    ...
    save_failed_queries_csv(
        ...
        spreadsheet,
        str(tmp_path),
        {0: "Accession failed, fallback also failed"},
...
def test_imageqr_with_fallback_query(tmp_path, orthanc):
    ...
    assert result["num_images_saved"] == 2, (
        f"Should save 2 images, saved {result['num_images_saved']}"
    )
