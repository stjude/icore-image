...
<e en="T" t="00080050" n="AccessionNumber">@hashPtID(@UID(),13)</e>
    ...
    assert len(dcm_files_in_output) == 2, (
        "Deidentified DICOM files should be preserved in output_dir"
    )
...
<e en="T" t="00080050" n="AccessionNumber">@hashPtID(@UID(),13)</e>
    ...
    dcm_files_in_output = list(output_dir.rglob("*.dcm"))
    assert len(dcm_files_in_output) == 1, (
        "Deidentified DICOM files should be preserved in output_dir"
    )
    ...
    if quarantine_dir.exists():
        ...
        for qfile in quarantine_files:
            assert "quarantine" in str(qfile), (
                "Quarantined files should be in quarantine subdirectory"
            )
...
<e en="T" t="00100010" n="PatientName">@empty()</e>
        ...
        dcm_files_in_output = list(output_dir.rglob("*.dcm"))
        assert len(dcm_files_in_output) == 4, (
            "Deidentified DICOM files should be preserved in output_dir"
        )
