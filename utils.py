...
def _build_mrn_date_query_and_filter(mrn, study_date, date_window_days):
    ...
    query_params = {"PatientID": mrn, "StudyDate": f"{start_date_str}-{end_date_str}"}
...
def generate_queries_and_filter(
    spreadsheet, date_window_days=0, use_fallback_query=False
):
    query_params_list = []
    ...
    for i, row in spreadsheet.dataframe.iterrows():
        if spreadsheet.acc_col and pd.notna(row.get(spreadsheet.acc_col)):
            ...
            if use_fallback_query and spreadsheet.mrn_col and spreadsheet.date_col:
                ...
                if (
                    pd.notna(mrn)
                    and pd.notna(study_date)
                    and isinstance(study_date, pd.Timestamp)
                ):
                    _, filter_condition = _build_mrn_date_query_and_filter(
                        str(mrn), study_date, date_window_days
                    )
                    filter_conditions.append(filter_condition)
        elif (
            spreadsheet.mrn_col
            and spreadsheet.date_col
            and pd.notna(row.get(spreadsheet.mrn_col))
            and pd.notna(row.get(spreadsheet.date_col))
        ):
            mrn = str(row[spreadsheet.mrn_col])
            study_date = row[spreadsheet.date_col]
            if not isinstance(study_date, pd.Timestamp):
                raise ValueError(
                    f"StudyDate must be in Excel date format (pd.Timestamp), got {type(study_date).__name__}: {study_date}"
                )

            query_params, filter_condition = _build_mrn_date_query_and_filter(
                mrn, study_date, date_window_days
            )
            query_params_list.append(query_params)
            filter_conditions.append(filter_condition)
        else:
            raise ValueError(
                "Row must have either acc_col or both mrn_col and date_col with valid values"
            )
...
def validate_date_window_days(date_window_days):
    if date_window_days < 0 or date_window_days > 10:
        raise ValueError(
            f"date_window_days must be between 0 and 10, got {date_window_days}"
        )


def save_failed_queries_csv(
    ...
    if use_fallback_query and has_accession and has_mrn and has_date:
        ...
        raise ValueError(
            "Spreadsheet must have either acc_col or both mrn_col and date_col"
        )
    ...
    for pacs in pacs_list:
        try:
            ...
            if result["success"]:
                ...
                logging.warning(
                    f"Failed to ping PACS {pacs.host}:{pacs.port} (AE: {pacs.aet}): {result['message']}"
                )
                continue
        except Exception as e:
            logging.warning(
                f"Failed to ping PACS {pacs.host}:{pacs.port} (AE: {pacs.aet}): {e}"
            )
            continue
    return valid_pacs_list


def find_studies_from_pacs_list(
    pacs_list,
    query_params_list,
    application_aet,
    ...
    for pacs in pacs_list:
        ...
        for i, query_params in enumerate(query_params_list):
            ...
            try:
                ...
                for result in results:
                    ...
                    if i in expected_accessions_map:
                        ...
                        if result_acc != expected_acc:
                            logging.debug(
                                f"Rejecting study {study_uid}: AccessionNumber '{result_acc}' does not match expected '{expected_acc}'"
                            )
                            continue

                    study_pacs_map[study_uid] = (pacs, i)
                    logging.debug(
                        f"Found study {study_uid} on PACS {pacs.host}:{pacs.port}"
                    )
                ...
                if not results:
                    logging.warning(
                        f"No studies found for query {i + 1}: {query_params}"
                    )
            except Exception as e:
                logging.exception(
                    f"Failed to find studies for query {i + 1}: {str(e)}. Moving on."
                )
...
def _attempt_fallback_queries(
    ...
    logging.info(
        f"Fallback: {len(failed_indices)} accession queries failed, checking fallback data (mrn_col='{spreadsheet.mrn_col}', date_col='{spreadsheet.date_col}')"
    )

    if not (spreadsheet.mrn_col and spreadsheet.date_col):
        logging.warning(
            "Fallback: mrn_col or date_col is empty/None, cannot attempt fallback"
        )
        for i in failed_indices:
            failure_details[i] = (
                failure_details.get(i, "Failed to find images")
    ...
    if spreadsheet.mrn_col not in df_columns:
        logging.warning(
            f"Fallback: mrn_col '{spreadsheet.mrn_col}' not found in spreadsheet columns: {df_columns}"
        )
    if spreadsheet.date_col not in df_columns:
        logging.warning(
            f"Fallback: date_col '{spreadsheet.date_col}' not found in spreadsheet columns: {df_columns}"
        )
    ...
    for original_index in failed_indices:
        ...
        if (
            ...
            mrn = str(mrn)
            fallback_query, _ = _build_mrn_date_query_and_filter(
                mrn, study_date, date_window_days
            )
            ...
            logging.debug(
                f"Fallback: row {original_index} → MRN={mrn}, DateRange={fallback_query['StudyDate']}"
            )
        else:
            logging.warning(
                f"Fallback: row {original_index} skipped - mrn={mrn} (notna={pd.notna(mrn)}), date={study_date} (notna={pd.notna(study_date)}, is_timestamp={isinstance(study_date, pd.Timestamp)})"
            )
            failure_details[original_index] = (
                failure_details.get(original_index, "Failed to find images")
                + " (no fallback data available)"
            )

    if not fallback_queries:
        logging.warning(
            "Fallback: no valid fallback queries could be generated from failed rows"
        )
        return study_pacs_map, failed_indices, failure_details

    logging.info(
        f"Attempting fallback MRN+date queries for {len(fallback_queries)} failed accession queries"
    )
    ...
    for original_index in failed_indices:
        ...
        if original_index in fallback_failed_original:
            final_details[original_index] = (
                "Accession query failed, fallback MRN+date query also failed"
            )
        elif (
            original_index not in failure_details
            or "no fallback data" in failure_details.get(original_index, "")
        ):
            final_details[original_index] = failure_details.get(
                original_index, "Failed to find images"
            )
        else:
            final_details[original_index] = (
                "Accession query failed, fallback MRN+date query also failed"
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            ...
            try:
                ...
                logging.warning(
                    "Failed to read file '%s' while counting DICOM files: %s",
                    file_path,
                    e,
                )
                continue
...
def _parse_date_value(value):
    ...
    date_formats = [("%Y%m%d", 8), ("%Y-%m-%d", 10), ("%m/%d/%Y", 10)]
    ...
    if is_date_column:
        for idx, value in df[tag_name].items():
            if pd.notna(value) and not isinstance(value, (pd.Timestamp, datetime)):
                raise ValueError(
                    f"Column {tag_name} has inconsistent date types at row {idx}: expected datetime, got {type(value).__name__}"
                )
...
def format_dicom_date(date_value):
    if isinstance(date_value, pd.Timestamp):
        ...
        raise ValueError(
            f"Cannot format non-date value: {date_value} (type: {type(date_value).__name__})"
        )
