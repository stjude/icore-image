...
def validate_sas_url_endpoint(request):
    try:
        ...
        if not sas_url_param:
            return JsonResponse(
                {"valid": False, "error": "SAS URL is required"}, status=400
            )
        ...
        if result.get("error") is not None:
            return JsonResponse(
                {"valid": False, "error": "Validation error during SAS URL check"},
                status=400,
            )
    ...
    try:
        ...
        if "se" in query_params:
            ...
            try:
                ...
                return {
                    "valid": False,
                    "error": "Invalid expiry date format in SAS token",
                }
...
def get_protocol_settings(request, protocol_id):
    try:
        settings_dir = os.path.join(
            os.path.expanduser("~"), "Documents", "iCore", "config"
        )
        protocol_path = os.path.join(settings_dir, "protocol.xlsx")
        ...
        if matching_rows.empty:
            return JsonResponse(
                {"error": f"Protocol ID {protocol_id} not found"}, status=404
            )
...
def load_filters_from_protocol(protocol_row):
    ...
    return {"general_filters": general_filters, "modality_filters": modality_filters}
...
def verify_admin_password(request):
    ...
    try:
        ...
        if is_valid:
            AUTH_LOGGER.info(
                f"Authentication successful for AET Title: {aet} and hostname: {request.META.get('REMOTE_ADDR')}"
            )
        else:
            AUTH_LOGGER.info(
