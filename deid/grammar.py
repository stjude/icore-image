...
def build_tag_dict():
    ...
    if getattr(sys, "frozen", False):
        ...
        mapping_path = (
            ...
            / "resources"
            / "pydicom_ctp_tag_dictionary.xml"
        )
...
def generate_anonymizer_script(
    tags_to_keep,
    tags_to_dateshift,
    tags_to_randomize,
    date_shift_days,
    ...
    tags_to_dateshift = [
        tag.strip() for tag in tags_to_dateshift.split("\n") if tag.strip()
    ]
    tags_to_randomize = [
        tag.strip() for tag in tags_to_randomize.split("\n") if tag.strip()
    ]
    ...
    for tag_name in sorted(set(tags_to_keep + tags_to_dateshift + tags_to_randomize)):
        ...
        if tag_name in tags_to_keep:
            ...
            script.append(
                f'   <e en="T" t="{tag}" n="{name}">@incrementdate(this,@DATEINC)</e>'
            )
...
def generate_lookup_contents(lookup_file):
    ...
    if "AccessionNumber" in df.columns:
        ...
        raise ValueError(
            "Lookup file must contain either AccessionNumber or MRN column"
        )
    ...
    for mapping in config.mappings:
        ...
        for _, row in df.iterrows():
            if isinstance(config.trigger_col, list):
                trigger_val = "|".join(
                    [str(row[t]).strip() for t in config.trigger_col]
                )
            else:
        ...
        tag_hex = (
            tag_dict[mapping["tag"]].replace("(", "").replace(",", "").replace(")", "")
        )
        if isinstance(trigger_tag, list):
            trigger_hex = [
                tag_dict[t].replace("(", "").replace(",", "").replace(")", "")
                for t in trigger_tag
            ]
            ...
            trigger_hex = (
                tag_dict[trigger_tag].replace("(", "").replace(",", "").replace(")", "")
            )
            script_lines.append(
                f'   <e en="T" t="{tag_hex}" n="{mapping["tag"]}">@lookup({trigger_hex}, {keytype})</e>'
            )
...
def generate_lookup_contents_legacy(lookup_file):
    ...
    for output_tag, chunk in df.groupby("OutputTag", sort=False):
        ...
        try:
            output_tag = (
                tag_dict[output_name].replace("(", "").replace(",", "").replace(")", "")
            )
        except KeyError:
...
def generate_hipaa_safe_harbor_script(site_id, date_shift_days):
    ...
    script.append(
        '   <e en="T" t="00081060" n="NameOfPhysiciansReadingStudy">@empty()</e>'
    )
    script.append(
        '   <e en="T" t="00081072" n="OperatorIdentificationSequence">@remove()</e>'
    )
    ...
    for tag_hex, tag_name in config["tags_to_dateshift"]:
        script.append(
            f'   <e en="T" t="{tag_hex}" n="{tag_name}">@incrementdate(this,@DATEINC)</e>'
        )
    ...
    script.append(
        '   <e en="T" t="00101050" n="InsurancePlanIdentification">@remove()</e>'
    )
    ...
    for tag_hex, tag_name in config["tags_to_randomize"]:
        script.append(
            f'   <e en="T" t="{tag_hex}" n="{tag_name}">@hashuid(@UIDROOT,this)</e>'
        )
