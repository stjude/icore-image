...
class TestTranslateAnonymizerScript:
    ...
    def test_incrementdate_action(self):
        ...
        assert "JITTER StudyDate var:DATEINC" in lines
    ...
    def test_param_action(self):
        ...
        lines, params = translate_anonymizer_script(xml)
        assert "REPLACE ClinicalTrialSponsorName var:SITEID" in lines
    ...
    def test_disabled_elements_skipped(self):
        ...
        lines, _ = translate_anonymizer_script(xml)
        assert any("SpecificCharacterSet" in l for l in lines)
        assert not any("ImageType" in l for l in lines)
        assert not any("Modality" in l for l in lines)
...
Test Label
  ...
  (0,0,512,100)
        ...
        assert "KEEP SpecificCharacterSet" in recipe
        assert "REMOVE InstitutionName" in recipe
