...
<e en="T" t="00100010" n="PatientName">@empty()</e>
    ...
    assert result["num_images_quarantined"] == 0, (
        "No images should be quarantined from empty directory"
    )
...
def test_mapping_file_invalid_dicom_tag(tmp_path):
    ...
    df = pd.DataFrame(
        {"InvalidTagName": ["value1"], "New-InvalidTagName": ["newvalue1"]}
    )
    df.to_excel(mapping_file, index=False)
...
def test_mapping_file_missing_new_column(tmp_path):
    ...
    df = pd.DataFrame({"AccessionNumber": ["ACC001", "ACC002"]})
    df.to_excel(mapping_file, index=False)
...
def test_sc_pdf_routed_to_quarantine_by_default(tmp_path):
    ...
    sc_ds = _create_secondary_capture_dicom(
        patient_id="MRN002", patient_name="Doe^Jane", accession="ACC002"
    )
...
<e en="T" t="00100020" n="PatientID">@empty()</e>
    ...
    output_files = list(output_dir.rglob("*.dcm"))
    assert len(output_files) == 1, (
        f"Expected 1 CT file in output, found {len(output_files)}"
    )

    for f in output_files:
        ds = pydicom.dcmread(f)
        assert not ds.SOPClassUID.startswith("1.2.840.10008.5.1.4.1.1.7"), (
            "SC file should not be in output"
        )

    quarantine_dir = appdata_dir / "quarantine"
    quarantine_files = list(quarantine_dir.rglob("*.dcm"))
    assert len(quarantine_files) >= 1, (
        f"Expected SC file in quarantine, found {len(quarantine_files)}"
    )
...
<e en="T" t="00100020" n="PatientID">@empty()</e>
    ...
    assert len(custom_files) >= 1, (
        f"Expected SC file in custom dir, found {len(custom_files)}"
    )
...
<e en="T" t="00080060" n="Modality">@keep()</e>
    ...
    assert len(output_files) == 3, (
        f"Expected 3 CT files in output, found {len(output_files)}"
    )
...
<e en="T" t="00100020" n="PatientID">@empty()</e>
    ...
    quarantine_files = list(quarantine_dir.rglob("*.dcm"))
    assert len(quarantine_files) >= 1, (
        f"Expected PDF in quarantine, found {len(quarantine_files)}"
    )
...
<e en="T" t="00100020" n="PatientID">@empty()</e>
    ...
    quarantine_files = list(quarantine_dir.rglob("*.dcm"))
    assert len(quarantine_files) >= 1, (
        f"Expected burned-in file in quarantine, found {len(quarantine_files)}"
    )
