...
def test_imagedeid_pacs_with_accession_filter(tmp_path, orthanc):
    ...
    for i in range(3):
        ds = _create_test_dicom(
            f"ACC{i:03d}", f"MRN{i:04d}", f"Smith^John{i}", "CT", "0.5"
        )
    ...
    for i in range(3, 6):
        ds = _create_test_dicom(
            f"ACC{i:03d}", f"MRN{i:04d}", f"Smith^John{i}", "CT", "3.0"
        )
        ds.InstanceNumber = i + 1
        _upload_dicom_to_orthanc(ds, orthanc)
    ...
    for i in range(6, 9):
        ds = _create_test_dicom(
            f"ACC{i:03d}", f"MRN{i:04d}", f"Smith^John{i}", "MR", "3.0"
        )
        ds.InstanceNumber = i + 1
        _upload_dicom_to_orthanc(ds, orthanc)
...
<e en="T" t="00080050" n="AccessionNumber">@hashPtID(@UID(),13)</e>
    ...
    for file in output_files:
        ...
        assert (
            not hasattr(ds, "ReferringPhysicianName") or ds.ReferringPhysicianName == ""
    ...
    if "PatientName" in deid_df.columns:
        ...
        assert all(name in ["", "nan", '=""'] for name in patient_names), (
            "PatientName should be empty"
        )
    if "PatientID" in deid_df.columns:
        patient_ids = deid_df["PatientID"].astype(str)
        assert all(pid in ["", "nan", '=""'] for pid in patient_ids), (
    ...
    assert len(found_accessions) >= 1, (
        f"At least one of the expected accession numbers should be in linker. Found: {original_accessions_in_linker}, Expected: {expected_accessions}"
    )
    ...
    for _, row in linker_df.iterrows():
        ...
        orig_acc = (
            str(row["Original AccessionNumber"])
            .strip()
            .strip('="')
            .strip("(")
            .strip(")")
        ...
        if trial_acc and orig_acc and trial_acc != "nan" and orig_acc != "nan":
            assert trial_acc != orig_acc, (
                f"Trial accession should be different from original: {trial_acc} vs {orig_acc}"
            )
...
<e en="T" t="00100010" n="PatientName">@empty()</e>
    ...
    assert result["failed_query_indices"] == [0, 1, 2], (
        "Failed indices should be [0, 1, 2]"
    )
...
def test_imagedeid_filter_script_generation(tmp_path, orthanc):
    ...
    with (
        patch(
            "utils.find_studies_from_pacs_list"
...
def test_imagedeid_multiple_pacs(tmp_path):
    ...
    with get_free_port() as storescp_port:
        ...
        orthanc2 = OrthancServer(aet="ORTHANC2", storescp_port=storescp_port)
        orthanc2.start()
    ...
    try:
        ...
        pacs_configs = [
            PacsConfiguration(
                host="localhost", port=orthanc1.dicom_port, aet=orthanc1.aet
            ),
            PacsConfiguration(
                host="localhost", port=orthanc2.dicom_port, aet=orthanc2.aet
            ),
        ]
        ...
        assert result["num_images_saved"] == 4, (
...
<e en="T" t="00080020" n="StudyDate">@keep()</e>
    ...
    assert result["num_studies_found"] >= 2, (
        f"Should find at least 2 studies within 2-day window, found {result['num_studies_found']}"
    )
    assert result["num_images_saved"] >= 2, (
        f"Should save at least 2 images, saved {result['num_images_saved']}"
    )
    assert "20250101" in study_dates_found and "20250103" in study_dates_found, (
        f"Should find both studies within 2-day window. Found dates: {study_dates_found}"
    )
    assert "20250110" not in study_dates_found, (
        f"Should not find study outside window. Found dates: {study_dates_found}"
    )
...
def test_imagedeid_pacs_deid_pixels_parameter(tmp_path, orthanc):
    ...
    assert result["num_images_saved"] == 1, (
        f"Should save 1 image, saved {result['num_images_saved']}"
    )
...
<e en="T" t="00100020" n="PatientID">@empty()</e>
    ...
    assert result_without_filter["num_images_quarantined"] == 0, (
        "Without default filter, no images should be quarantined"
    )
    ...
    assert result_with_filter["num_images_quarantined"] == 1, (
        "With default filter, derived CT should be quarantined"
    )
    ...
    assert "DERIVED" in quarantine_ds.ImageType, (
        "Quarantined file should be the derived CT"
    )
...
<e en="T" t="00080020" n="StudyDate">@keep()</e>
    ...
    assert unmapped_file.AccessionNumber == "", (
        "ACC999 should be empty (fallback to @empty())"
    )
    assert unmapped_file.PatientID == "MRN0999", (
        "MRN0999 should be kept (fallback to @keep())"
    )
    assert unmapped_file.StudyDate == "20231231", (
        "Unmapped study date should be kept (fallback to @keep())"
    )
...
def test_imagedeid_pacs_fallback_to_simple_action(tmp_path, orthanc):
    ...
    for i, (acc, mrn) in enumerate(
        [("ACC001", "MRN0000"), ("ACC002", "MRN0001"), ("ACC003", "MRN0002")]
    ):
        ds = _create_test_dicom(acc, mrn, f"Patient{i}", "CT", "3.0")
        ds.InstanceNumber = i + 1
        _upload_dicom_to_orthanc(ds, orthanc)
...
<e en="T" t="00080050" n="AccessionNumber">@empty()</e>
    ...
    for file in output_files:
        ...
        if ds.AccessionNumber == "MAPPED001":
            assert ds.PatientID == "NEWMRN0000", (
                "Mapped file should have mapped PatientID"
            )
        else:
            assert ds.AccessionNumber == "", (
                "Unmapped accession should be empty (fallback to @empty())"
            )
            assert ds.PatientID in ["MRN0001", "MRN0002"], (
                "Unmapped PatientID should be kept (fallback to @keep())"
            )
...
<e en="T" t="00100020" n="PatientID">@empty()</e>
    ...
    assert "ACC003" in accession_numbers, (
        "ACC003 should be kept (not in mapping, fallback to @keep() for new tags)"
    )
...
<e en="T" t="00080050" n="AccessionNumber">@lookup(this,AccessionNumber,keep)</e>
    ...
    assert ds.AccessionNumber == "FROM_EXPLICIT", (
        "Explicit lookup_table should override mapping_file_path"
    )
    assert ds.AccessionNumber != "FROM_MAPPING", (
        "Mapping file should be ignored when explicit lookup_table is provided"
    )
...
def test_imagedeid_pacs_saves_failed_queries_csv(tmp_path, orthanc):
    ...
    query_file = appdata_dir / "query.xlsx"
    query_df = pd.DataFrame(
        {"AccessionNumber": ["ACC001", "ACC999"], "PatientID": ["MRN001", "MRN999"]}
    )
    query_df.to_excel(query_file, index=False)
